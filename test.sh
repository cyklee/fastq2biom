...
for file in *merged.fastq; do
	...
	usearch9 -fastq_filter "${sampleID}_merged.fastq" -fastq_maxee 1.0 -fastqout "${sampleID}_filtered.fastq" -relabel @ -log ${sampleID}_filter.log
	usearch9 -fastq_filter "${sampleID}_merged.fastq" -fastaout "${sampleID}_unfiltered.fasta" -relabel @ -log ${sampleID}_unfilter.log
done
